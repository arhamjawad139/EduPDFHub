...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Attach Event Listeners for Terms & Conditions
    setupTermsModal();
});
...
/**
 * Injects and manages the Terms & Conditions Modal
 */
function setupTermsModal() {
    // Target links matching terms page or explicit IDs
    const termsLinks = document.querySelectorAll('a[href*="Terms-and-Conditions"], #openTermsBtn');

    if (termsLinks.length === 0) return;

    // Create Modal Elements dynamically if not present
    if (!document.getElementById("termsModalContainer")) {
        const modalHTML = `
            <div id="termsModalContainer" class="terms-modal-overlay" style="display: none;">
                <div class="terms-modal-card glass">
                    <div class="terms-modal-header">
                        <h3><i class="bi bi-shield-check"></i> Terms & Conditions</h3>
                        <button id="closeTermsBtn" class="close-icon-btn" aria-label="Close Modal">&times;</button>
                    </div>
                    <div class="terms-modal-body">
                        <iframe src="Terms-and-Conditions.html" title="Terms and Conditions" frameborder="0"></iframe>
                    </div>
                </div>
            </div>
        `;
    ...
    const modalOverlay = document.getElementById("termsModalContainer");
    const closeBtn = document.getElementById("closeTermsBtn");

    // Open Modal Handlers
    termsLinks.forEach(link => {
        link.addEventListener("click", (e) => {
            // Prevent navigating away if viewing on desktop/in-page
...
/**
 * Dynamically injects modal CSS into document head
 */
function injectModalStyles() {
    const styleId = "terms-modal-styles";
